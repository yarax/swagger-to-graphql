...
var hasNoProperties = function hasNoProperties(jsonSchema) {
  return !(0, _keys2.default)(jsonSchema.properties || {}).length;
};
...
var createGQLObject = exports.createGQLObject = function createGQLObject(jsonSchema, title, isInputType, gqlTypes) {
  ...
  if (hasNoProperties(jsonSchema) && _buildOptions.buildOptions.includes(_buildOptions.BUILD_OPTIONS.EmptyToJSON)) {
    return _graphqlTypeJson.GraphQLJSON;
  }
...
var getPrimitiveTypes = function getPrimitiveTypes(jsonSchema) {
  ...
  if (_buildOptions.buildOptions.includes(_buildOptions.BUILD_OPTIONS.NaN) && jsonType === 'number') {

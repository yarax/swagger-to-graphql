...
exports.createGQLObject = function (jsonSchema, title, isInputType, gqlTypes) {
    ...
    if (isArrayType(jsonSchema)) {
        if (isRefType(jsonSchema.items)) {
            return new graphql_1.GraphQLList(graphql_1.GraphQLNonNull(getExistingType(jsonSchema.items.$ref, isInputType, gqlTypes)));
        }
        if (isObjectType(jsonSchema.items) || isArrayType(jsonSchema.items)) {
            return new graphql_1.GraphQLList(graphql_1.GraphQLNonNull(exports.createGQLObject(jsonSchema.items, title + "_items", isInputType, gqlTypes)));
        }
        return new graphql_1.GraphQLList(graphql_1.GraphQLNonNull(getPrimitiveTypes(jsonSchema.items)));
    }

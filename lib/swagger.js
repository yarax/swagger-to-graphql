...
var getParamDetails = function (param) {
    var resolvedParam = param;
    var name = replaceOddChars(resolvedParam.name);
    var type = resolvedParam.type;
    return __assign({ name: name }, (type && { type: type }), { jsonSchema: resolvedParam });
};
...
exports.getAllEndPoints = function (schema) {
    ...
    Object.keys(schema.paths).forEach(function (path) {
        ...
        Object.keys(route).forEach(function (method) {
            ...
            //
            if (obj.parameters) {
                parameterDetails = obj.parameters.map(function (param) { return getParamDetails(param); });
            }
            else {
                parameterDetails = [];
            }
